 ...
 * @author peter
 */
public class PrivateSearchResultAdapter extends BaseAdapter implements View.OnClickListener {
        ...
        if (!TextUtils.isEmpty(flag)) {
            switch (flag) {
                case PRIVATEFUNDACTIVITY:
                    ...
                    // 专户类型
                    holder.tvSignNumber.setText(pofManagersBeans.get(position).getMandatorName());
                    // 备案编码
                    holder.tvSignAddress.setText(Tools.timeStamp2Date(pofManagersBeans.get(position).getEstablishDate() + "", ""));
                    // 备案日期
