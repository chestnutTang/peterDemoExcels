...
public class ExcelActivity extends BaseActivity implements View.OnClickListener {
    ...
    private File file;
    private String[] title = {"日期", "食物支出", "日用品项", "交通话费", "旅游出行", "穿着支出", "医疗保健", "人情客往",
            "宝宝专项", "房租水电", "其它支出", "备注说明"};
...
//        setContentView(R.layout.activity_excel);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.family_bill_save) {
            saveData = new String[]{new SimpleDateFormat("yyyy-MM-dd").format(new Date()),
                    ...
                    (), mTrafficEdt.getText().toString().trim(), mTravelEdt.getText().toString()
                    .trim(), mClothesEdt.getText().toString().trim(), mDoctorEdt.getText()
                    .toString().trim(), mRenQingEdt.getText().toString().trim(), mBabyEdt.getText
                    ().toString().trim(), mLiveEdt.getText().toString().trim(), mOtherEdt.getText
                    ().toString().trim(), mRemarkEdt.getText().toString().trim()};
            if (canSave(saveData)) {
    ...
    public void initData() {
        ...
        makeDir(file);
        ExcelUtils.initExcel(file.toString() + "/测试表格.xls", title);
        ExcelUtils.writeObjListToExcel(getBillData(), getSDPath() + "/Family/测试表格.xls", this);
    }
